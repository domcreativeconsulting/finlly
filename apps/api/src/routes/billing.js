...
/** Rate limiter for billing status polling: 180 req/15min per user/IP (covers 5s interval × 5min + buffer) */
...
router.post('/webhooks/asaas', webhookLimiter, async (req, res, next) => {
  ...
  try {
    const result = await processarWebhookAsaas(
      payload,
      rawBody,
      signatureHeader
    );
...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      const { assinante, paymentLink, pixQrCode, pixCopiaECola } =
        await criarAssinatura(req.user.sub, {
          ...req.body,
          remoteIp,
        });
...
router.post(
  ...
  async (req, res, next) => {
    ...
    try {
      ...
      logger.info(
        { summary, userId: req.user.sub },
        'Reconciliação manual disparada'
      );

...
export default function CheckoutPage() {
  ...
  const initialPlan = PLANS[searchParams.get('plano')]
  ...
  const initialStep =
    Number.isInteger(stepParam) && stepParam >= 1 ? stepParam : 1;
  ...
  function handleChange(e) {
    ...
    const masks = {
      cpf: applyMaskCpf,
      telefone: applyMaskPhone,
      cardNumber: applyMaskCard,
      cardExpiry: applyMaskExpiry,
    };
    setForm((p) => ({
      ...p,
  ...
  async function handlePay(e) {
    ...
    if (method === 'CARTAO') {
      ...
      if (form.cardNumber.replace(/\D/g, '').length < 16)
        return setError('Número do cartão inválido.');
      if (form.cardExpiry.replace(/\D/g, '').length < 4)
        return setError('Validade inválida.');
    ...
    if (isLoading) {
      setError(
        'Aguarde um momento enquanto verificamos sua sessão. Tente novamente em alguns segundos.'
      );
      return;
    }
    ...
    try {
      ...
      if (!isAuthenticated) {
        ...
        if (!form.nome.trim() || !form.email.trim() || !form.senha) {
          throw new Error(
            'Dados de cadastro incompletos. Preencha nome, e-mail e senha.'
          );
        }
      ...
      const payloadBase = {
        ...
        telefone:
          (form.telefone || '').replace(/\D/g, '') || usuario?.telefone || '',
        ...(form.cupomCodigo?.trim()
          ? { cupomCodigo: form.cupomCodigo.trim() }
          : {}),
      };

      if (!payloadBase.nome || payloadBase.nome.length < 3) {
        throw new Error(
          'Nome inválido. Por favor complete seu nome no formulário.'
        );
      }
      if (
        !payloadBase.email ||
        !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(payloadBase.email)
      ) {
        throw new Error(
          'E-mail inválido. Por favor verifique seu e-mail no formulário.'
        );
      }
        ...
        try {
          ...
          const isPaid = Boolean(
            ...
            (assinanteAtual.status === 'ativo' ||
              String(assinanteAtual.asaasStatus || '')
                .toLowerCase()
                .includes('pago') ||
              String(assinanteAtual.status || '')
                .toLowerCase()
                .includes('pago'))
          );
          ...
          if (elapsed >= PAYMENT_TIMEOUT_MS) {
            ...
            paymentPollRef.current = null;
            setWaitingPayment(false);
            setPaymentPollingError(
              'Não foi possível confirmar o pagamento automaticamente. Verifique após alguns minutos ou entre em contato com o suporte.'
            );
            toast.warn(
              'Ainda sem confirmação de pagamento. Verifique seu e-mail ou tente novamente.'
            );
          }
        } catch (err) {
      ...
      {!isMobile && (
        ...
        >
          ...
          <div>
            ...
            >
              Contas fixas e variáveis, recebimentos, investimentos e metas —
              com apoio do agente.
            </p>
            <div
            ...
            >
              {[
                {
                  icon: faShieldHalved,
                  text: 'Bloqueio automático por inadimplência',
                },
                { icon: faArrowsRotate, text: 'Recorrência via Asaas' },
                {
                  icon: faBullseye,
                  text: 'Metas + anexos (extratos/comprovantes)',
                },
              ].map(({ icon, text }) => (
                <div
                  key={text}
              ...
              {[1, 2, 3].map((s) => (
                <div
                  ...
                  style={{
                    ...
                    borderRadius: '4px',
                    background:
                      s === step ? '#1e3a5f' : s < step ? '#c4e91f' : '#e5e7eb',
                    transition: 'all 0.3s',
                  }}
                />
              ))}
          ...
          {step === 2 && (
            <div
              style={{
                ...
                borderRadius: '20px',
                padding: '28px',
                boxShadow: '0 20px 48px rgba(4,10,24,0.55)',
              }}
            >
              <StepPagamento
                form={form}
    ...
    >
      {error && (
        ...
        >
          ...
          {error === EMAIL_DUPLICADO ? (
            ...
            >
              E-mail já cadastrado. Faça login para assinar.
            </Link>
          ) : (
            error
          )}
      ...
      <FieldLight label="Cupom">
        ...
        <span style={{ fontSize: '12px', color: '#6b7280', marginTop: '4px' }}>
          Se o cupom ativar <strong>teste</strong>, o acesso é{' '}
          <strong>liberado na hora</strong>.
        </span>
      </FieldLight>
  ...
  return (
    ...
    >
      ...
      <div>
        ...
        <p style={{ margin: 0, fontSize: '13px', color: C.inkSoft }}>
      ...
      >
        ...
        >
          <span style={{ fontSize: '15px', color: C.ink, fontWeight: '600' }}>
            {plan.label}
          </span>
          <span
            style={{ fontSize: '20px', fontWeight: '800', color: C.accent }}
          >
            {plan.price}
          </span>
        </div>
        <div style={{ fontSize: '12px', color: C.inkSoft, marginTop: '4px' }}>
          Recorrência {plan.ciclo}
        </div>
      </div>
      <div>
        <div
      ...
      {method === 'PIX' && (
        <div
          style={{
            ...
            color: C.inkSoft,
            padding: '12px 14px',
            background: 'rgba(196,233,31,0.06)',
            border: `1px solid rgba(196,233,31,0.2)`,
            borderRadius: '10px',
          }}
        >
          <span>ℹ️</span>
          <span>
            O QR Code PIX será gerado na próxima tela para você escanear ou
            copiar o código.
          </span>
        </div>
      )}
      <button
        type="submit"
        disabled={loading || Boolean(waitingPayment)}
        style={{
          width: '100%',
          padding: '14px 20px',
          borderRadius: '999px',
      ...
      >
        {loading
          ...
          : method === 'PIX'
            ? `🔒 Gerar QR Code PIX — ${plan.price}`
            : `🔒 Pagar ${plan.price} com cartão`}
      </button>
    </form>
  );
}
    ...
    >
      <div
        style={{
          ...
          height: '64px',
          borderRadius: '50%',
          background: 'rgba(196,233,31,0.15)',
      ...
      {waitingPayment && (
        ...
        >
          ...
          <div style={{ fontSize: 13, marginTop: 6 }}>
            Assim que o pagamento for confirmado, atualizaremos seu acesso
            automaticamente.
          </div>
        </div>
      )}
      ...
      >
        ...
        >
          <span style={{ fontSize: '14px', color: C.inkSoft }}>
            {plan.label}
          </span>
          <span
            style={{ fontSize: '18px', fontWeight: '800', color: C.accent }}
          >
            {plan.price}
          </span>
        </div>
            ...
            <div style={{ width: '100%' }}>
              ...
              <button
                ...
                style={{
                  ...
                  boxShadow: copied
                    ? 'none'
                    : '0 10px 24px rgba(196,233,31,0.28)',
                  fontFamily: 'inherit',
                  transition: 'all 0.2s',
                }}
              >
      ...
      {method !== 'PIX' && (
        ...
        >
          ✅ Pagamento enviado para processamento. Você receberá um e-mail de
          confirmação.
        </div>
      )}
      ...
      >
        ...
        >
          ...
          <strong style={{ color: C.ink }}>{email}</strong>. Clique no link
          antes de fazer login. Verifique também a pasta de spam.
        </p>
      </div>

...
function VencimentoBadge({ dateStr }) {
  ...
  if (days === null) return <span style={{ fontWeight: 600, color: colors.neutral800 }}>—</span>;
  if (days < 0) return <span style={{ fontWeight: 600, color: '#dc2626', fontSize: 14 }}>{formatDate(dateStr)} · Vencido</span>;
  if (days <= 7) return <span style={{ fontWeight: 600, color: '#d97706', fontSize: 14 }}>{formatDate(dateStr)} · {days}d restantes</span>;
  return <span style={{ fontWeight: 600, color: colors.neutral800, fontSize: 14 }}>{formatDate(dateStr)}</span>;
}
...
export default function BillingStatusPage() {
  ...
  useEffect(() => {
    billingService
      ...
      .catch((err) => {
        const msg = err?.response?.data?.message || err?.response?.data?.error || 'Erro ao carregar status da assinatura.';
        setError(msg);
      })
  ...
  async function handleCancel() {
    ...
    try {
      ...
      const msg = err?.response?.data?.message || err?.response?.data?.error || 'Erro ao cancelar assinatura.';
      toast.error(msg);
    } finally {
      setCancelling(false);
    }
  ...
  return (
    <InadimplenteGuard>
      <div style={{ display: 'flex', minHeight: '100vh', backgroundColor: colors.bg, fontFamily: tokens.fontFamily }}>
        <AppSidebar sidebarOpen={sidebarOpen} currentPath="/assinatura" isExpanded={sidebarExpanded} onHoverChange={setSidebarExpanded} />
          ...
          <main style={{ flex: 1, padding: '32px 32px 32px 24px' }}>
            ...
            {loading && (
              <div style={{ display: 'flex', alignItems: 'center', padding: '16px 0' }}>
                <span style={{ display: 'inline-block', width: '22px', height: '22px', border: `3px solid ${colors.neutral200}`, borderTopColor: colors.primaryLight, borderRadius: radius.full, animation: 'spin 0.7s linear infinite', flexShrink: 0 }} />
                <span style={{ color: colors.neutral500, marginLeft: '12px', fontSize: typography.sizes.md }}>Carregando...</span>
              </div>
            )}

            {!loading && error && (
              <div style={{ padding: '12px 16px', backgroundColor: colors.errorBg, border: `1px solid ${colors.errorBorder}`, borderRadius: radius.md, color: colors.error, fontSize: typography.sizes.md }}>
                {error}
              </div>
            )}

            {!loading && !error && !assinante && (
              <div style={{ backgroundColor: colors.surface, borderRadius: radius.lg, padding: '32px', boxShadow: shadows.sm, display: 'flex', flexDirection: 'column', alignItems: 'flex-start', gap: '16px', maxWidth: '480px' }}>
                <h2 style={{ margin: 0, fontSize: typography.sizes['4xl'], fontWeight: typography.weights.semibold, color: colors.neutral800 }}>Assinatura</h2>
                <p style={{ margin: 0, fontSize: typography.sizes.md, color: colors.neutral500 }}>Você não possui uma assinatura ativa.</p>
                <Button onClick={() => navigate('/checkout')}>Assinar um plano →</Button>
              </div>
            )}
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px', alignItems: 'start' }}>
                ...
                <div style={{ backgroundColor: colors.surface, borderRadius: radius.lg, padding: '28px', boxShadow: shadows.sm }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '4px' }}>
                    <h2 style={{ margin: 0, fontSize: typography.sizes['4xl'], fontWeight: typography.weights.semibold, color: colors.neutral800 }}>Assinatura</h2>
                    <Badge variant={assinante.status === 'ativo' ? 'success' : 'neutral'}>{assinante.status ?? '—'}</Badge>
                  </div>

                  <p style={{ margin: '0 0 20px', fontSize: typography.sizes.base, color: colors.neutral500 }}>
                    Seu acesso ao sistema depende de pagamento recorrente confirmado no Asaas.
                  </p>

                  {assinante.status === 'ativo' && (
                    <div style={{ padding: '10px 14px', backgroundColor: colors.successBg, border: `1px solid ${tokens.color?.successBorder ?? '#bbf7d0'}`, borderRadius: radius.md, color: colors.successText, fontSize: typography.sizes.base, fontWeight: typography.weights.medium, marginBottom: '20px' }}>
                  ...
                  <dl style={{ margin: 0, padding: 0 }}>
                    {[
                      { label: 'Plano', value: <span style={{ fontWeight: 600, color: colors.neutral800, textTransform: 'capitalize' }}>{assinante.plano ?? '—'}</span> },
                      { label: 'Ciclo', value: <CicloBadge ciclo={assinante.ciclo} /> },
                      { label: 'Forma de pagamento', value: <PagamentoBadge forma={assinante.formaPagamento} /> },
                      { label: 'Próximo vencimento', value: <VencimentoBadge dateStr={assinante.dataProximoVencimento} /> },
                    ].map(({ label, value }) => (
                      <div key={label} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 0', borderBottom: `1px solid ${colors.neutral100}` }}>
                        <dt style={{ fontSize: typography.sizes.md, color: colors.neutral500, fontWeight: typography.weights.medium, margin: 0 }}>{label}</dt>
                ...
                <div style={{ backgroundColor: colors.surface, borderRadius: radius.lg, padding: '28px', boxShadow: shadows.sm }}>
                  <h2 style={{ margin: '0 0 8px', fontSize: typography.sizes['4xl'], fontWeight: typography.weights.semibold, color: colors.neutral800 }}>Manter Plano Ativo</h2>
                  <p style={{ margin: '0 0 8px', fontSize: typography.sizes.base, color: colors.neutral500 }}>

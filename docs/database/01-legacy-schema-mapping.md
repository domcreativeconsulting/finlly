...
| Billing & Assinaturas      | `plans`, `subscriptions`, `payments`, `coupons`                                       | 4   |
...
#### Tabela `plans`
**Nome proposto (Postgres):** *unificada em `assinantes`*  
...
| `coupon_id`      | `INT`           | NULL     | NULL        | FK implícita → `coupons.id`           |
...
- FKs para `users`, `plans`, `coupons` não declaradas — integridade violável
...
| `subscriptions`         | `coupon_id`      | `coupons`           | N:1          | Implícita     | Nenhum        |
...
| `attachment_relations`  | `entity_id`      | *(polimórfico)*     | N:1          | Nenhuma       | Nenhum        |
...
| 12| Ausência de UNIQUE em `code` (`coupons`)    | Cupons duplicados aceitos silenciosamente                |
...
*Próximos documentos: `02-relationships-matrix.md`, `03-legacy-issues.md`*

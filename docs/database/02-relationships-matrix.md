...
| `subscriptions` | `plan_id`         | `plans`         | N:1          | Muitos→Um     | ⚠️ Implícita | —              | _(unificado — N/A)_   |
| `subscriptions` | `coupon_id`       | `cupons`        | N:1          | Muitos→Um     | ⚠️ Implícita | —              | `SET NULL`            |
...
| `attachment_relations` | `entity_id`     | _(polimórfico)_ | N:1          | Muitos→Um     | ❌ Polimórfica | —              | CHECK constraint em `entity_type` |
...
| `subscriptions.coupon_id → cupons.id`         | `checkout_assinatura.php`       | Busca de cupom por ID antes de aplicar desconto                 |
...
| `investments.bank_id → banks.id`              | _(schema inference)_            | JOIN em listagem de investimentos                               |
| `goal_movements.goal_id → goals.id`           | _(schema inference)_            | SUM por `goal_id` para calcular progresso                       |
...
_Documentos relacionados: `01-legacy-schema-mapping.md`, `03-legacy-issues.md`_
